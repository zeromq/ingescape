...
//
void print_usage() {
    printf("Usage example: puncher --verbose --port %d --device device_name\n", port);
...
void busCommand(void){
    ...
    assert(igs_busSendStringToAgent("sparingPartner", "test igs_busSendStringToAgent") == IGS_SUCCESS);
    assert(igs_busSendDataToAgent("sparingPartner", myOtherData, 64) == IGS_SUCCESS);
    msg = zmsg_new();
    zmsg_addstr(msg, "message content");
    assert(igs_busSendZMQMsgToAgent("sparingPartner", &msg) == IGS_SUCCESS);
}
...
void busCommandSparing(void){
    ...
    assert(igs_busSendStringToAgent("puncher", "test igs_busSendStringToAgent") == IGS_SUCCESS);
    assert(igs_busSendDataToAgent("puncher", myOtherData, 64) == IGS_SUCCESS);
    msg = zmsg_new();
    zmsg_addstr(msg, "message content");
    assert(igs_busSendZMQMsgToAgent("puncher", &msg) == IGS_SUCCESS);
}
...
void editorCommand(const char *agentUUID, const char *input){
    ...
    zmsg_addstr(msg, "MAP");
    zmsg_addstr(msg, input);
    zmsg_addstr(msg, "sparingPartner");
...
void securityCommand(void){ //for ingescape puncher
    igs_stop();
    
    assert(igs_enableSecurity(NULL, NULL) == IGS_SUCCESS);
    assert(igs_enableSecurity("toto", NULL) == IGS_FAILURE);
    assert(igs_enableSecurity("certificates/puncher.cert_secret", "toto") == IGS_FAILURE);
    assert(igs_enableSecurity("certificates/puncher.cert_secret", "certificates") == IGS_SUCCESS);
    
    assert(igs_brokerAdd("tcp://10.0.0.8:5661") == IGS_FAILURE);
    assert(igs_brokerAddSecure("tcp://10.0.0.8:5661", "toto") == IGS_FAILURE);
    assert(igs_brokerAddSecure("tcp://10.0.0.8:5661", "certificates/sparingPartner.cert") == IGS_SUCCESS);
...
void securityCommandSparing(void){
    ...
    igs_enableSecurity("certificates/sparingPartner.cert_secret", "certificates");
    
    igs_brokerAddSecure("tcp://10.0.0.8:5660", "certificates/puncher.cert");
    igs_enableAsBroker("tcp://10.0.0.8:5661");
