...
bool puncher_firstAgentEntered = false;
bool puncher_firstAgentKnowsUs = false;
bool puncher_firstAgentExited = false;
...
void agentEvent(igs_agent_event_t event, const char *uuid, const char *name, void *eventData, void *myData){
    ...
    if (streq(name, "firstAgent")){
        if (event == IGS_AGENT_ENTERED)
            puncher_firstAgentEntered = true;
        if (event == IGS_AGENT_KNOWS_US)
            puncher_firstAgentKnowsUs = true;
        if (event == IGS_AGENT_EXITED)
            puncher_firstAgentExited = true;
    }else if(streq(name, "secondAgent")){
        if (event == IGS_AGENT_ENTERED)
            puncher_secondAgentEntered = true;
        if (event == IGS_AGENT_KNOWS_US)
            puncher_secondAgentKnowsUs = true;
        if (event == IGS_AGENT_EXITED)
            puncher_secondAgentExited = true;
    }
}
...
bool first_puncherAgentEntered = false;
bool first_punchergentKnowsUs = false;
bool first_puncherAgentExited = false;
...
void agentEvent2(igs_agent_t *agent, igs_agent_event_t event, const char *uuid, const char *name, void *eventData, void *myData){
    ...
    if (agent == firstAgent){
        if (streq(name, "puncher")){
            if (event == IGS_AGENT_ENTERED)
                first_puncherAgentEntered = true;
            if (event == IGS_AGENT_KNOWS_US)
                first_punchergentKnowsUs = true;
            if (event == IGS_AGENT_EXITED)
                first_puncherAgentExited = true;
        }else if(streq(name, "secondAgent")){
            if (event == IGS_AGENT_ENTERED)
        ...
        if (streq(name, "puncher")){
            if (event == IGS_AGENT_ENTERED)
                second_puncherAgentEntered = true;
            if (event == IGS_AGENT_KNOWS_US)
                second_punchergentKnowsUs = true;
            if (event == IGS_AGENT_EXITED)
                second_puncherAgentExited = true;
...
void puncherBusCallback(const char *event, const char *peerID, const char *name,
                        const char *address, const char *channel,
                        zhash_t *headers, zmsg_t *msg, void *myData){
    if (autoTests && autoTestsHaveStarted){
        assert(streq(name, "sparingPartner"));
        if (msg){
            printf("new message:\n");
            zmsg_print(msg);
        }
        ...
        if (streq(event, "WHISPER")){
            char *s = zmsg_popstr(msg);
            if(streq(name, "sparingPartner") && streq(s, "starting autotests")){
...
//callbacks for iops
void puncherIOPCallback(iop_t iopType, const char* name, iopType_t valueType, void* value, size_t valueSize, void* myData){
    if (autoTestsHaveStarted){
        switch (valueType) {
            case IGS_BOOL_T:
...
//    free(exportedMapping);
    ...
    assert(igs_initCall("myCall", puncherCallCallback, NULL) == IGS_SUCCESS);
    ...
    igs_observeBus(puncherBusCallback, NULL);
    ...
    igs_initCall("myCall", puncherCallCallback, NULL);
    ...
    igs_observeInput("my_impulsion", puncherIOPCallback, NULL);
    igs_observeInput("my_bool", puncherIOPCallback, NULL);
    igs_observeInput("my_int", puncherIOPCallback, NULL);
    igs_observeInput("my_double", puncherIOPCallback, NULL);
    igs_observeInput("my_string", puncherIOPCallback, NULL);
    igs_observeInput("my_data", puncherIOPCallback, NULL);
    
    igs_addMappingEntry("my_impulsion", "sparingPartner", "sparing_impulsion");
    igs_addMappingEntry("my_bool", "sparingPartner", "sparing_bool");
    igs_addMappingEntry("my_int", "sparingPartner", "sparing_int");
    igs_addMappingEntry("my_double", "sparingPartner", "sparing_double");
    igs_addMappingEntry("my_string", "sparingPartner", "sparing_string");
    igs_addMappingEntry("my_data", "sparingPartner", "sparing_data");
    ...
    igsAgent_addMappingEntry(firstAgent, "first_impulsion", "sparingPartner", "sparing_impulsion");
    igsAgent_addMappingEntry(firstAgent, "first_bool", "sparingPartner", "sparing_bool");
    igsAgent_addMappingEntry(firstAgent, "first_int", "sparingPartner", "sparing_int");
    igsAgent_addMappingEntry(firstAgent, "first_double", "sparingPartner", "sparing_double");
    igsAgent_addMappingEntry(firstAgent, "first_string", "sparingPartner", "sparing_string");
    igsAgent_addMappingEntry(firstAgent, "first_data", "sparingPartner", "sparing_data");
    ...
    igsAgent_addMappingEntry(secondAgent, "second_impulsion", "sparingPartner", "sparing_impulsion");
    igsAgent_addMappingEntry(secondAgent, "second_bool", "sparingPartner", "sparing_bool");
    igsAgent_addMappingEntry(secondAgent, "second_int", "sparingPartner", "sparing_int");
    igsAgent_addMappingEntry(secondAgent, "second_double", "sparingPartner", "sparing_double");
    igsAgent_addMappingEntry(secondAgent, "second_string", "sparingPartner", "sparing_string");
    igsAgent_addMappingEntry(secondAgent, "second_data", "sparingPartner", "sparing_data");
    
    igsAgent_addMappingEntry(secondAgent, "second_impulsion", "puncher", "my_impulsion");
    igsAgent_addMappingEntry(secondAgent, "second_bool", "puncher", "my_bool");
    igsAgent_addMappingEntry(secondAgent, "second_int", "puncher", "my_int");
    igsAgent_addMappingEntry(secondAgent, "second_double", "puncher", "my_double");
    igsAgent_addMappingEntry(secondAgent, "second_string", "puncher", "my_string");
    igsAgent_addMappingEntry(secondAgent, "second_data", "puncher", "my_data");
    ...
    assert(puncher_firstAgentEntered);
    assert(puncher_firstAgentKnowsUs);
    assert(first_puncherAgentEntered);
    assert(first_punchergentKnowsUs);
    igsAgent_activate(secondAgent);
    assert(second_firstAgentEntered);
    assert(second_firstAgentKnowsUs);
    assert(second_puncherAgentEntered);
    assert(second_punchergentKnowsUs);
    assert(puncher_secondAgentEntered);
    assert(puncher_secondAgentKnowsUs);
    assert(first_secondAgentEntered);
    assert(first_secondAgentKnowsUs);
    igsAgent_deactivate(firstAgent);
    assert(puncher_firstAgentExited);
