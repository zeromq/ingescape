...
int runAutoTests(zloop_t *loop, int timer_id, void *arg){
    ...
    zclock_sleep(1500);
    igs_busSendStringToAgent("tester", "starting autotests");
    zclock_sleep(100);
    ...
    busCommandSparing();
    zclock_sleep(250);
    igs_busSendStringToAgent("tester", "STOP_PEER");
    return -1;
}
