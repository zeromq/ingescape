...
CALL vcvarsall.bat x86 -vcvars_ver=14.0

cd %CURRENTDIR%\libsodium\builds\msvc\build
CALL buildbase.bat ..\vs2015\libsodium.sln 15
ECHO
cd %CURRENTDIR%

cd libzmq\builds\msvc\
cd build
CALL buildbase.bat ..\vs2015\libzmq.sln 15
cd %CURRENTDIR%
...
CALL configure.bat
cd build
CALL buildbase.bat ..\vs2015\zyre.sln 15
cd %CURRENTDIR%
