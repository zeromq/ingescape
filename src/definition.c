...
agent_iop_t* definition_createIop(igsAgent_t *agent, const char *name, iop_t type, iopType_t value_type, void *value, size_t size){
    ...
    switch (type) {
        case IGS_INPUT_T:
            if (definition_addIopToDefinition(iop, IGS_INPUT_T, agent->internal_definition) < 1){
                model_readWriteLock();
            ...
            break;
        case IGS_OUTPUT_T:
            if (definition_addIopToDefinition(iop, IGS_OUTPUT_T, agent->internal_definition) < 1){
                model_readWriteLock();
                definition_freeIOP(iop);
                model_readWriteUnlock();
                return NULL;
            ...
            break;
        case IGS_PARAMETER_T:
            if (definition_addIopToDefinition(iop, IGS_PARAMETER_T, agent->internal_definition) < 1){
                model_readWriteLock();
                definition_freeIOP(iop);
                model_readWriteUnlock();
                return NULL;
...
void igsAgent_writeDefinitionToPath(igsAgent_t *agent){
    ...
    if (fp == NULL){
        igs_error("Could not open %s for writing", agent->definition_path);
    }else{
