...
void igs_setVerbose(bool verbose){
    initInternalAgentIfNeeded();
    igsAgent_setVerbose(internalAgent, verbose);
}

bool igs_isVerbose(void){
    initInternalAgentIfNeeded();
    return igsAgent_isVerbose(internalAgent);
}

void igs_setUseColorVerbose(bool useColor){
    initInternalAgentIfNeeded();
    igsAgent_setUseColorVerbose(internalAgent, useColor);
}

bool igs_getUseColorVerbose(void){
    initInternalAgentIfNeeded();
    return igsAgent_getUseColorVerbose(internalAgent);
}

void igs_setLogStream(bool useLogStream){
    initInternalAgentIfNeeded();
    igsAgent_setLogStream(internalAgent, useLogStream);
}

bool igs_getLogStream(void){
    initInternalAgentIfNeeded();
    return igsAgent_getLogStream(internalAgent);
}

void igs_setLogInFile(bool useLogFile){
    initInternalAgentIfNeeded();
    igsAgent_setLogInFile(internalAgent, useLogFile);
}
 //enable logs in file
bool igs_getLogInFile(void){
    initInternalAgentIfNeeded();
    return igsAgent_getLogInFile(internalAgent);
}

void igs_setLogPath(const char *path){
    initInternalAgentIfNeeded();
    igsAgent_setLogPath(internalAgent, path);
}

char* igs_getLogPath(void){
    initInternalAgentIfNeeded();
    return igsAgent_getLogPath(internalAgent);
}

void igs_setLogLevel (igs_logLevel_t level){
    initInternalAgentIfNeeded();
    igsAgent_setLogLevel(internalAgent, level);
}
 //set log level in console, default is IGS_LOG_INFO
igs_logLevel_t igs_getLogLevel(void){
    initInternalAgentIfNeeded();
    return igsAgent_getLogLevel(internalAgent);
}
...
void igs_log(igs_logLevel_t level, const char *function, const char *format, ...){
    ...
    vsnprintf(logContent, 2047, format, list);
    va_end(list);
    igsAgent_log(internalAgent, level, function, "%s", logContent);
}
