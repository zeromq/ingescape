...
int publishOutput(const char* output_name)
{
    ...
    if(agentElements->publisher != NULL && found_iop != NULL)
    {
        ...
        {
            char* str_value = strdup(iop_value_to_string(found_iop));
    ...
    if (item->revents & ZMQ_POLLIN)
    {
        ...
        if (streq (event, "ENTER")){
            ...
            while ((k = (char *)zlist_pop(keys))) {
                ...
                *(insert + 1) = '\0';
                ...
                if (strcmp(k, "publisher")==0){
                    ...
                    subscriber = (subscriber_t*)calloc(1, sizeof(subscriber_t));
                    ...
                    subscriber->pollItem = (zmq_pollitem_t*)calloc(1, sizeof(zmq_pollitem_t));
            ...
            if(strlen(message) > strlen(EXPORT_DEFINITION_PREFIX))
            {
                ...
                {
                    char* strDefinition = (char*)calloc(strlen(message)- strlen(EXPORT_DEFINITION_PREFIX)+1, sizeof(char));
