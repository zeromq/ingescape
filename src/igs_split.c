                             ...
                             const char *with_output)
{
    ...
    if (streq (reviewed_to_agent, a_name))
        igsagent_warn (agent, "split inputs to outputs of the same agent will not work EXCEPT from one clone or variant to others");
    free (a_name);
    ...
    if (!split) {
        ...
        igs_split_t *new = split_create_split_element (reviewed_from_our_input, reviewed_to_agent, reviewed_with_output);
        new->id = hash;
        zlist_append(agent->mapping->split_elements, new);
