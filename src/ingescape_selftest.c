/*  =========================================================================
    ingescape_selftest.c - run selftests
...
// Tests for stable public classes:
    { "igs_json", igs_json_test, true, true, NULL },
    { "igs_json_node", igs_json_node_test, true, true, NULL },
    { "igs_service_arg", igs_service_arg_test, true, true, NULL },
    { "igs", igs_test, true, true, NULL },
    { "igsagent", igsagent_test, true, true, NULL },
...
{
    test_item_t *item;
    printf ("Running ingescape selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq (item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef INGESCAPE_BUILD_DRAFT_API // selftest is still in draft
        else
            ingescape_private_selftest (verbose, item->subtest);
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("ingescape_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running ingescape test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef INGESCAPE_BUILD_DRAFT_API // selftest is still in draft
        else
            ingescape_private_selftest (verbose, test->subtest);
#endif // INGESCAPE_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
